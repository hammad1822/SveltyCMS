...
const PROTECTED_COLLECTIONS = ['users', 'sessions', 'tokens', 'roles', 'system_settings', 'private_settings', 'audit_logs'];
 ...
 */
export function createScopedDbAdapter(dbAdapter: IDBAdapter, pluginId: string): { adapter: IDBAdapter; stats: { queryCount: number } } {
	...
	function validateCollection(collection: string, operation: string): void {
		...
		if (PROTECTED_COLLECTIONS.includes(collection.toLowerCase())) {
			throw new Error(`Plugin "${pluginId}" denied access to protected collection "${collection}" (${operation})`);
		}
		...
		if (writeOps.includes(operation) && !collection.startsWith(prefix)) {
			throw new Error(`Plugin "${pluginId}" can only write to collections prefixed with "${prefix}" (attempted: "${collection}")`);
		}
 ...
 */
export function createSandboxedContext(context: PluginContext, pluginId: string): { context: PluginContext; stats: { queryCount: number } } {
		...
		try {
			const result = await Promise.race([
				...
				new Promise((_, reject) =>
					setTimeout(() => reject(new Error(`Plugin "${pluginId}" hook "${hookName}" timed out (${HOOK_TIMEOUT_MS}ms)`)), HOOK_TIMEOUT_MS)
				)
			]);
	...
	return async (ctx: PluginContext, entries: Record<string, unknown>[]) => {
		...
		try {
			return await Promise.race([
				...
				new Promise((_, reject) =>
					setTimeout(() => reject(new Error(`Plugin "${pluginId}" SSR hook timed out (${HOOK_TIMEOUT_MS}ms)`)), HOOK_TIMEOUT_MS)
				)
			]);
		} catch (error) {
