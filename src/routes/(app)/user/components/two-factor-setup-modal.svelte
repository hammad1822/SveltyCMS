<!--
@file src/routes/(app)/user/components/two-factor-setup-modal.svelte
@component
...
<div class="modal-example-form {cBase}">
				...
				<div>
					...
					<div class="relative">
						<input
							...
							oninput={handleInput}
							placeholder={twofa_code_placeholder()}
			...
			{#if backupCodes.length > 0}
				<div class="preset-ghost-warning-500 rounded-lg border p-4">
					<div class="mb-3 flex items-start gap-3">
						...
						<div class="flex-1">
							<h5 class="mb-1 font-semibold">{twofa_backup_codes_title()}</h5>
					...
					<div class="flex items-start gap-2 rounded bg-warning-500/10 p-3">
						...
						<p class="text-sm text-warning-600 dark:text-warning-400">{twofa_backup_codes_warning()}</p>
					</div>
				</div>
			{/if}
		</div>
	...
	<footer class="modal-footer flex items-center justify-between p-4 {parent?.regionFooter ?? ''}">
		{#if currentStep === 'setup'}
			<!-- Setup Footer -->
			<button type="button" class="preset-outlined-secondary-500 btn" onclick={cancelSetup}>{button_cancel()}</button>
			<button
				type="submit"
				form="twofa-form"
