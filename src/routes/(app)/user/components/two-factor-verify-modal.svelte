<!--
@file src/routes/(app)/user/components/two-factor-verify-modal.svelte
@component
...
<div class="max-w-md p-6">
	...
	<div class="mb-6">
		<div class="relative">
			<input
				...
				placeholder={useBackupCode ? twofa_backup_code_placeholder() : twofa_code_placeholder()}
	...
	<div class="mt-4 text-center">
		<div class="space-y-1 text-xs text-surface-500">
			{#if !useBackupCode}
				<p>{twofa_help_authenticator()}</p>
			{:else}
				<p>{twofa_help_backup()}</p>
			{/if}
		</div>
	</div>
</div>
