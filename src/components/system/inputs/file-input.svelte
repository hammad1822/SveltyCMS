	...
	>
		<div class="grid grid-cols-6 items-center p-4">
			...
			<div class="col-span-5">
				{#if !show}
					<p class="font-bold">
						<span class="text-tertiary-500 dark:text-primary-500">{widget_ImageUpload_Upload()}</span>
						{widget_ImageUpload_Drag()}
					</p>
				{:else}
					<p class="font-bold">
						<span class="text-tertiary-500 dark:text-primary-500">{widget_ImageUpload_Replace()}</span>
						{widget_ImageUpload_Drag()}
					</p>
				{/if}
				<p class="text-sm opacity-75">{widget_ImageUpload_Allowed()}.</p>

				<div class="flex w-full justify-center gap-2">
					<button
						onclick={openFileInput}
						aria-label={widget_ImageUpload_BrowseNew()}
					...
					<button
						...
						aria-label={widget_ImageUpload_SelectMedia()}
						class="preset-filled-tertiary-500 btn mt-3 dark:preset-filled-primary-500"
					>
						{widget_ImageUpload_SelectMedia()}
					</button>
	...
	{#if showMedia}
		...
		>
			<div class="bg-surface-100-800-token flex items-center justify-between border-b p-2">
				<p class="ml-auto font-bold text-black dark:text-primary-500">{widget_ImageUpload_SelectImage()}</p>
